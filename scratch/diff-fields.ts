...
function main() {
  const agentSet = new Set(COMPANY_FIELDS);
  const dbSet = new Set(dbFields);

  console.log(`Agent fields: ${COMPANY_FIELDS.length}`);
  console.log(`Database fields: ${dbFields.length}`);

  const missingInAgent = dbFields.filter(f => !agentSet.has(f));
  console.log(`\nFields in Database but missing from Agent (${missingInAgent.length}):`);
  console.log(JSON.stringify(missingInAgent, null, 2));

  const extraInAgent = COMPANY_FIELDS.filter(f => !dbSet.has(f));
  console.log(`\nFields in Agent but missing from Database (${extraInAgent.length}):`);
  console.log(JSON.stringify(extraInAgent, null, 2));
}
