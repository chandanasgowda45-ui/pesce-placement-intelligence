...
function main() {
  console.log(`VALID_SUPABASE_FIELDS size: ${VALID_SUPABASE_FIELDS.size}`);
  console.log(`Database fields count: ${dbFields.length}`);

  const inDbNotRegistry = dbFields.filter(f => !VALID_SUPABASE_FIELDS.has(f as any));
  console.log(`\nFields in Database but NOT in Schema Registry (${inDbNotRegistry.length}):`);
  console.log(JSON.stringify(inDbNotRegistry, null, 2));

  const inRegistryNotDb = Array.from(VALID_SUPABASE_FIELDS).filter(f => !dbFields.includes(f));
  console.log(`\nFields in Schema Registry but NOT in Database (${inRegistryNotDb.length}):`);
  console.log(JSON.stringify(inRegistryNotDb, null, 2));
}
